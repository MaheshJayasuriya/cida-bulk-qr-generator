...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['links'], $_POST['event_name'])) {
    $eventName = trim($_POST['event_name']);  // Get event name
    $eventName = preg_replace('/[^a-zA-Z0-9_]/', '_', $eventName);  // Sanitize the event name

    $links = explode("\n", $_POST['links']);  // Split the text area by line
    $links = sanitizeLinks($links);           // Sanitize the links

    // Create the event-specific output directory
    $outputDir = '../qr_codes/' . $eventName . '/';
    if (!is_dir($outputDir)) {
